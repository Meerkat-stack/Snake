    ...
    void render() {
        ...
        for (int y = 0; y < rows; ++y) {
            for (int x = 0; x < cols; ++x) {
                ...
                if ((x + y) % 2 == 0)
                    bgRect.setFillColor(sf::Color(Basil_color));//Rysowanie planszy
                else
        ...
        for (size_t i = 0; i < snake.size(); ++i) {
            ...
            if (i == 0) gameRect.setFillColor(sf::Color(0, 97, 255));//Rysowanie węża
            else gameRect.setFillColor(sf::Color(0, 80, 220));//Rysowanie jedzenia
        ...
        if (isGameOver) {//Wyśietla ekran po przegranej
            sf::RectangleShape overlay(sf::Vector2f(WINDOW_WIDTH, WINDOW_HEIGHT));
            overlay.setFillColor(sf::Color(0, 0, 0, 75));//Przyciemnia tło
            window->draw(overlay);
            ...
            subText2.setOrigin({ rect.position.x + rect.size.x / 2.0f, rect.position.y + rect.size.y / 2.0f });
